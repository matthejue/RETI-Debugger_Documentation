...
As luv is actually an external library that was made available in Neovim over the subtable \inlinebox{vim.loop}, the internals of it won't be explained in too much detail as there's an overwehlming amount of theory connected with this topic. But as \inlinebox{vim.loop} is technivally in spite of everything a part of Neovim and as the plugin made extensive use of this library there's a special section \ref{sec:asynchronous execution} in the appendix explaining different particularities of using asynchronous execution in Neovim and listing all the different functions from \inlinebox{vim.loop} used in the project.
...
And whenever the menu for choosing an example program was started, after waiting for the user to submit a menu entry, the execution was resumed by using \inlinebox{new_async(...)} and \inlinebox{async_send(...)}. After that reading the content of the submitted example file was accomplished by using \inlinebox{fs_open(...)}, \inlinebox{fs_stat(...)}, \inlinebox{fs_read(...)} and \inlinebox{fs_close(...)}. 

Updating the size of the window layout whenever the size of the terminal window running Neovim changes, was achieved using a timer via \inlinebox{new_timer()} and \inlinebox{timer:start(...)} and testing every second if the size of the terminal changed. If the size of the terminal changed, the size of the window layout was updated with the current size of the terminal.
