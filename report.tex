...
In case the instruction the \inlinebox{PC} register currently pointed at when the \inlinebox{next} command was issued was a \inlinebox{CALL PRINT <reg>} instruction the RETI-Interpreter sends the output via \inlinebox{print(f"Output: \{reg\_value\}")} in form of the f-string \inlinebox{f"Output: \{reg\_value\}"} and afterwards sends the registers on top of it via \inlinebox{print(reti.regs_str() + chr(4))}.
