...
\begin{itemize}
  ...
  \item \inlinebox{new_async(vim.schedule_wrap(callback) -> async_event}:
  \item \inlinebox{async_send(async_event)}:
  ...
  \item \inlinebox{fs_read(fd, stat.size callback)}:  Asynchronously reads data from a file and calls the callback function when finished.
  \item \inlinebox{fs_close()}:
  \item \inlinebox{new_timer() -> timer}:
  \item \inlinebox{timer:start()}:
\end{itemize}
