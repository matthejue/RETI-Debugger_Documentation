...
In the exeuction process of the plugin it was often neccessary to wait for a certain event to happen, but then Neovim would actually freeze until this event happened or freeze forever if this event wouldn't ever happen. For this reason \alert{luv} was used to make any code that included waiting for a certain event asynchronous and non-blocking. The luv library makes \alert{libuv} available to lua scripts, whereby \alert{libuv} is a library to implement asynchronous behaviour. The \alert{luv} library is made available in Neovim via the subtable \inlinebox{vim.loop}.

The library is also able to start processes in own threads such that they can be executed parallel oder pseudo-parallel. When the RETI-Debugger is started the RETI-Intepreter is started in a own thread via the \inlinebox{spawn(...)} function. It is not possible to execute anything in parallel that depends on the Neovim API as Neovim is still up to this day single-threaded.
...
\begin{itemize}
  ...
  \end{itemize}
  \item If a \alert{popup} window, like an output or input window is \alert{visible} or the RETI-\alert{Interpreter terminated} it shouldn't be possible to execute the \inlinebox{next} action
    \begin{itemize}
...
The properties just listed are just a few examples of the properties ensured by the statemachine to give an idea. The RETI-Debugger does also have a few smaller statemachines that take care of other smaller tasks. Explaining these smaller statemachines would be too much detail for this report, because other things have to be priotised more and the report has a limitted number of permitted pages.

In the RETI-Debugger the statemachine for action control also wasn't implemented exactly like in figure \ref{fig:statemachine}, because often it is enough to only change the value of one variable, because the other variables never change when taking a transition associated with a certain input symbol and also not always all variables have to be checked to determine if a transition exists, because a certain input symbol might be associated with a small number of transitions or transitions that lead to the same state.
